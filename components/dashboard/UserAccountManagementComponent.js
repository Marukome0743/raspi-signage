...
function UserAccountManagementComponent() {
  ...
  const [_errorPart, _setErrorPart] = useState("")
    ...
    async function getInfoData() {
      ...
      setUsers(
        getUserList.map((item) => {
          const list = contents.map((content, _index) =>
  ...
  useEffect(() => {
    const areaList = contentList.map((_item) => false)
    setEmail("")
    setUserName("")
    setPassword("")
    setArea(areaList)
  ...
  const onClickCreate = async () => {
    try {
      ...
      //        return;
      //      }
      const list = area.reduce((acc, item, index) => {
        if (item) {
  ...
  const onClickUpdate = async (index) => {
    try {
      setProgress(true)
      const list = users[index].coverageArea.reduce((acc, item, i) => {
        if (item) {
  ...
  const _onClickResetAccountPassword = async (index) => {
    try {
      setProgress(true)
      const _uid = users[index].uid
                              ...
                              <FormControlLabel
                                ...
                                control={
                                  <Checkbox
                                    name={`area[${index}]`}
                                    checked={area[index]}
                                    onChange={(_e) => onChangeCheckBox(index)}
                                    style={{ width: "20px" }}
                                  />
                                }
                ...
                return (
                  ...
                  >
                    ...
                    <IconButton
                      ...
                      onClick={(_e) => onClickRemove(index)}
                    >
                      <CancelIcon />
                    </IconButton>
                  </Paper>
