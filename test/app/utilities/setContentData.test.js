...
const createBuilder = () => {
  const builder = {
    select: (_cols, opts) => {
      if (opts?.head) return Promise.resolve(state.countResult)
      return builder
    },
    insert: (params) => {
    ...
    then: (resolve) => resolve(state.queryResult),
  }
  return builder
}
  ...
  supabaseAdmin: {
    ...
    auth: {
      admin: {
        ...
        updateUserById: () =>
          Promise.resolve({ error: state.updateUserByIdError }),
      },
    },
  },
}))
...
describe("deleteContentsData", () => {
  test("soft-deletes content by setting deleted=true", async () => {
    const contents = [{ orderId: "o1" }, { orderId: "o2" }]
...
describe("createAccountData", () => {
  ...
  test("does not insert user when auth fails", async () => {
    ...
    await createAccountData("dup@example.com", "pass", {})
...
describe("updateAccountData", () => {
  ...
  test("aborts when signIn fails", async () => {
    ...
    state.table = null
    state.operation = null
    const user = { userName: "x", management: false, coverageArea: [] }
...
describe("resetAccountPassword", () => {
  test("updates password and sets pass_flg", async () => {
    state.updateUserByIdError = null
    await resetAccountPassword("uid-1", "newpass")
    expect(state.table).toBe("users")
    expect(state.operation).toBe("update")
    expect(state.params).toEqual({ pass_flg: true })
  })

  test("does not update user when admin update fails", async () => {
    state.updateUserByIdError = { message: "failed" }
    state.table = null
    state.operation = null
    await resetAccountPassword("uid-1", "newpass")
    expect(state.operation).toBeNull()
...
describe("Property: setContentOrder always targets orders table", () => {
  ...
  test("always upserts to orders table", async () => {
    await fc.assert(
      fc.asyncProperty(docIdArb, contentArb, async (docId, content) => {
        await setContentOrder(docId, content)
        expect(state.table).toBe("orders")
        expect(state.operation).toBe("upsert")
      }),
